...
const Profile = () => {
  ...
  async function handleSubmit(e) {
    e.preventDefault();
      // Check if the fields are not modified
      if (name === auth?.user?.name && email === auth?.user?.email && phone === auth?.user?.phone && address === auth?.user?.address && pin === auth?.user?.pin) {
        toast.error("No changes made to the profile");
        return;
      }
  
    if (address && (!pin || pin.length !== 6)) {
  ...
  return (
    <div className="grid  gap-4">
      ...
      <div className="">
        {/* //form */}
        <div className="text-customTooDark bg-darkColor flex items-center justify-center flex-col gap-8">
